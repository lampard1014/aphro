...
import (
    ...
    "github.com/lampard1014/aphro/Biz/merchant/MerchantServiceIMP"
    "fmt"
)
...
func main() {
    defer deferFunc()
    lis, err := net.Listen("tcp", MerchantServiceIMP.Port)
    if err != nil {
        log.Fatal(err)
    }
    ...
    merchantServicePB.RegisterMerchantServiceServer(s, new(MerchantServiceIMP.MerchantServiceIMP))
    err = s.Serve(lis)
    if err != nil {
        log.Fatal(err)
    }
